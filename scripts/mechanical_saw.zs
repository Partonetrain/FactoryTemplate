...
function sawBoth(input as IItemStack, strippedOutput as IItemStack, output as IItemStack) as void{ 
	sawLog(input, strippedOutput);
	sawStripped(strippedOutput, output);
}


function sawLog(input as IItemStack, output as IItemStack) as void{ 
	var recipeName = "mechanical_saw_log_" + input.getDefinition().getRegistryName().getPath();
	<recipetype:create:cutting>.addRecipe(recipeName, [output], input, 100);
}

function sawStripped(input as IItemStack, output as IItemStack) as void{ 
	var recipeName = "mechanical_saw_stripped_" + input.getDefinition().getRegistryName().getPath();
